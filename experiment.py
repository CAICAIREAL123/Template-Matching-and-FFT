...
def template_match_test():
    ...
    for img_fl, img_template_fl, label in zip(
        ...
        img_in = cv2.imread(INPUT_DIR + "{}.png".format(img_fl))
        img_template = cv2.imread(
            "INPUT_DIR + "{}.png".format(img_template_fl))
        ...
        for method in (
           ...
           # print(np.shape(img_template))
            bottom_right = (top_left[0]+np.shape(img_template)[1], top_left[1]+np.shape(img_template)[0])
            ...
            cv2.imwrite(OUTPUT_DIR + "{}-{}.jpg".format(method, label), im_out)
...
def compression_runner():
    ...
    #plt.imshow(compressed_frequency_img)
    cv2.imwrite( 'output/dog_compressed.jpg', img_compressed)
    print("!")
    cv2.imwrite( 'output/dog_compressed_frequency.jpg',
...
def low_pass_filter_runner():
    ...
    cv2.imwrite('output/cat_lpf.jpg', img_low_pass)
    cv2.imwrite('output/cat_lpf_frequency.jpg',
